...
export interface ProductJsonItem {
  productId: string;
  cardId: string;
  imageId: string;
  title: string;
...
const DEFAULT_APPLICATION_STEPS: ProductStep[] = [
...
export const PRODUCTS_CATALOG: ProductJsonItem[] = [
  // --- 1. CREDIT CARDS (Max 3) ---
  {
  ...
  // --- 2. LOAN CARDS (Max 3: Personal, Home, Car) ---
  {
  ...
  // --- 3. INSURANCE CARDS (Max 3: Health, Car, Travel) ---
  {
...
/**
 * Upload all product JSON items to live Firebase Firestore database
 */
export async function seedProductsToFirestore(): Promise<{ success: boolean; count: number }> {
  try {
    let count = 0;
    const categoryMap: Record<string, string> = {
      'prd_credit_cards': 'credit_cards',
      'prd_loan_cards': 'loan_cards',
      'prd_insurance_cards': 'insurance_cards',
    };

    for (const item of PRODUCTS_CATALOG) {
      const categoryId = categoryMap[item.productId] || 'other';
      // Document path: products/{categoryId}/items/{cardId}
      const docRef = doc(db, 'products', categoryId, 'items', item.cardId);
